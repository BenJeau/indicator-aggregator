...
impl Source for UrlHaus {
    ...
    async fn fetch_data(
        ...
        let data = if indicator.kind == IndicatorKind::Url {
            database::logic::url_haus::get_url_haus_by_url(&indicator.data, &state.pool).await?
        } else {
            database::logic::url_haus::get_url_haus_by_domain(&indicator.data, &state.pool).await?
        };
